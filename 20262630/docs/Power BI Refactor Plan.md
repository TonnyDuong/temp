...
|---|---|---|
| `EMS Fixed Fee Forecast.xlsx` | `CONTRACTS` | `dim_Project` (contract attributes — MSA Reference, Project Code, Billing Schedule Item, Entity, Project Type, SuperSector) and `fact_Revenue[Type=Forecast]` after unpivoting the indexed monthly forecast columns |
...
| | `31.12.2025` | Legacy snapshot/reference sheet only; do not use for live in-contract forecast values while `CONTRACTS` carries the indexed monthly schedule |
...
Reporting note:
- `HARP_DATA` is a distinct ingest source because its sheet structure differs from `31.12.2025`, but it must not surface as a separate report dataset or report column. In the semantic/reporting layer it is part of the same **in-contract forecast** stream as the other contract forecast rows.
...
4. **Add Column** → **Conditional Column** for `Contract Type`:
   ...
   - Else if `Account Code` equals `40013` or `40014` → `"Out of Contract"`
   - Else → `null`
   - **OK**. Set type to **Text**.
...
- Filter to `40013` or `40014` — `Contract Type` should be `Out of Contract`.
...
4. **Filter** `Account No.` to `40010`, `40011`, `40012`, `40013`, `40014` only (the revenue codes).
...
7. **Add Column** → **Custom Column** to derive `Contract Type` from the normalized account code:
   ```m
   if List.Contains({"40013", "40014"}, [#"Account No."]) then "Out of Contract" else "In Contract"
   ```
...
1. Open the `EMS Fixed Fee Forecast.xlsx` workbook source. The sheets we want are `CONTRACTS` for the standard in-contract forecast and `HARP_DATA` for the HARP project. Each has wide date columns that must be unpivoted. Do not use the `31.12.2025` sheet for live forecast values; it misses the indexed monthly contract schedule now held in `CONTRACTS`.
...
- Sum where `Type = "Actual"` and `Account Code` is `40013` or `40014` matches the OOC YTD revenue total.
...
4. Filter `Account No.` to `60201`, `60202`, and `60203` only.
...
- `Sum(Amount)` where `Type = "Actual"` and `Category = "Subcontractor"` should match the NetSuite total of accounts `60201 + 60202 + 60203`.
...
Sources to union:
1. **Subcontractor actuals**: from `FinanceOutput FY26.xlsx` → `FY2026` sheet, normalize `Account No.` with `_NormalizeAccountCode`, then filter to `Account No. IN {60201, 60202, 60203}`. Tag `Category=Subcontractor, Type=Actual`.
...
**Done when**: `fact_Cost` has rows for all five sources, `Sum(Amount)` filtered to `Category=Subcontractor, Type=Actual` matches NetSuite's 60201+60202+60203 total, and `Category=Staff, Type=Actual` matches the timesheet × rate total.
...
- `[Subcontractor Costs]` filtered to `Type = Actual` should equal the NetSuite total of `60201 + 60202 + 60203` for the YTD period.
...
- [ ] `[Subcontractor Costs]` matches the NetSuite total of `60201 + 60202 + 60203`.
...
- Account codes (40010/11/12 = In Contract revenue, 40013/40014 = OOC actuals, 40014 = OOC forecast, 60201/02/03 = Subcontractor) → audit § Revenue/Cost classification
