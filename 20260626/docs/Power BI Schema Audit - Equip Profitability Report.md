...
> 24 Jun 2026 amendment: OOC actual revenue now includes account codes `40013` and `40014`; OOC forecast remains the Additional Services stream using `40014`. The current sprint hierarchy is `Sector -> Contract -> Project Display`; Region split/filtering is deferred.
...
| `40013` | Out of contract | **Out of Contract** | confirmed by the 24 Jun 2026 YPS-04 actuals extract |
| `40014` | Additional services | **Out of Contract** | retained for OOC forecast and existing additional-services classification |
...
Reporting note:
- HARP may continue to be ingested from its own `HARP_DATA` helper because the worksheet shape differs from the other forecast sheet, but it should not appear as a separate report dataset or separate forecast column. In report measures it belongs inside the single **in-contract forecast** stream.
...
| | `Oo Contract` *(= Out-of-Contract)* | NetSuite — Account Codes 40013/40014 |
| | `Total Rev` | Sum |
...
| KPI: Out-of-contract revenue | ✅ | same, filtered by `dim_Accounts[Account Code] IN { 40013, 40014 }` | 40013 confirmed by the 24 Jun 2026 YPS-04 actuals extract; 40014 retained for existing additional-services classification |
...
q. **Revenue measures**:
   - `[Revenue] = CALCULATE(SUM(fact_FinanceFY2026[Sum of Amount]), dim_Accounts[Account Code] IN { 40010, 40011, 40012, 40013, 40014 })`
   - `[In-Contract Revenue] = CALCULATE([Revenue], dim_Accounts[Account Code] IN { 40010, 40011, 40012 })`
   - `[Out-of-Contract Revenue] = CALCULATE([Revenue], dim_Accounts[Account Code] IN { 40013, 40014 })`
r. **Cost measures**:
